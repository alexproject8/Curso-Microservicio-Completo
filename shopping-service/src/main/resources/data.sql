CREATE TABLE tbl_invoices ( id BIGINT AUTO_INCREMENT PRIMARY KEY, number_invoice VARCHAR(250) NOT NULL, description VARCHAR(250) NOT NULL, create_at TIMESTAMP, customer_id BIGINT, state VARCHAR(250);
DROP TABLE IF EXISTS tbl_invoice_items;
CREATE TABLE tbl_invoice_items ( invoice_id BIGINT, product_id BIGINT, quantity DOUBLE,price DOUBLE );
